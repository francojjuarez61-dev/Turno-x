...
function setLiquidPct(pct){
  const v = Math.max(0, Math.min(100, pct));
  ringBtn.style.setProperty('--liquid-pct', v.toFixed(2) + '%');
}
...
function setProgress(p01){
  ...
  // UI only: liquid starts full and drains to empty
  const fill = (1 - clamped) * 100;
  setLiquidPct(fill);

  // keep ringCap hidden (UI only)
...
function tick(){
  ...
  document.body.classList.remove('is-overtime');
    ...
    setMode('overtime');
    document.body.classList.add('is-overtime');
